...
镜像内运行端口为 3000，宿主机默认监听 `0.0.0.0:18088`。文档目录以只读
bind mount 挂载到 `/docs`；程序以非 root 用户运行，镜像不包含用户文档。
目录须可遍历、文件须可读。~~当前服务没有登录认证~~。当前版本强制后端登录认证，
...
后续更新宿主机文档目录即可由容器读取，无需重建镜像。不要把 Windows 的
